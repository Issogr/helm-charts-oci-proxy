...
- pushing a git tag like `v1.2.3` publishes a Docker image tagged `1.2.3` to `ghcr.io/<owner>/helm-charts-oci-proxy`
- the same git tag also publishes the Helm chart with `version: 1.2.3` and `appVersion: 1.2.3` to `ghcr.io/<owner>/helm-charts/helm-charts-oci-proxy`

This keeps Docker and Helm releases aligned, publishes only for explicit tagged releases, and uses GitHub Container Registry with the built-in `GITHUB_TOKEN` instead of custom registry credentials.
